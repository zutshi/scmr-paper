\subsection{PWA Relational Abstraction}

Given an abstraction, we can define its enrichment

We now briefly overview the approach, which consists of three
steps

Discover the abstraction 
enriches the abstraction
formulates a BMC query
concretizes
...
DEFINITION 3.4 (ABSTRACT STATE GRAPH). Let ∆ > 0 be
a fixed time step. The abstract state graph H(∆) for time step ∆
is defined by the set of vertices C, and edges (Ci, Cj ) whenever
Ci
∆ Cj . Let C0 denote the collection of initial cells in C, i.e., cells
Ci such that Ci ∩ X0 6= ∅. Further, let Cu denote the set of unsafe
...
\subsection{Enriched Graph Abstraction}
