...
\mypara{$k$-relational $\epsilon$-RTS ($(k,\epsilon)$-RTS)}
...
in~\algoref{krelabs}. The overall algorithm for computing piecewise
relational abstractions is presented in~\algoref{pwa-rel}.

\begin{algorithm}[t]
\DontPrintSemicolon
    \caption{$\mathsf{k-RelationalAbstraction}$\label{algo:krelabs}}
    \KwIn{$\System$, $\simmap_\Delta$, $\delta_{max}$ , regression kernel $F_k$}
    \KwOut{$\RTSAbstraction{\epsilon}{\System}$}
    \ForAll{$(\Cells,\Cells') \in \CellEdges$}{
    \While{$k \le k_{max}$ \and $\delta \ge \delta_{max}$}{
...
\KwIn{$\System$, $\simmap_\Delta$, $\epsilon$ , regression kernel $F_k$}
